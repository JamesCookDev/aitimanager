...
export interface ProgressBlockProps {
  value: number; maxValue: number; label: string; showLabel: boolean;
  showPercentage: boolean; barColor: string; trackColor: string; height: number;
  borderRadius: number; animated: boolean; striped: boolean; labelColor: string; labelFontSize: number;
}

export const ProgressBlock: UserComponent<Partial<ProgressBlockProps & LayoutProps>> = (props) => {
  const {
      ...
      >
        {showLabel && (
          <div className="flex items-center justify-between mb-2">
            ...
            {showPercentage && <span style={{ color: labelColor, fontSize: labelFontSize - 1, opacity: 0.7 }}>{Math.round(pct)}%</span>}
          </div>
        )}
        <div style={{ backgroundColor: trackColor, borderRadius, height, overflow: 'hidden', position: 'relative' }}>
          <div style={{
