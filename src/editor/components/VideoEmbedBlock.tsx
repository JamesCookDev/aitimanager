...
export interface VideoEmbedBlockProps {
  url: string; aspectRatio: '16:9' | '9:16' | '4:3' | '1:1';
  borderRadius: number; autoplay: boolean; muted: boolean; loop: boolean; opacity: number;
}

function getEmbedUrl(url: string, autoplay: boolean, muted: boolean, loop: boolean): string | null {
  try {
    const ytMatch = url.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([\w-]+)/);
    if (ytMatch) { const p = new URLSearchParams(); if (autoplay) p.set('autoplay','1'); if (muted) p.set('mute','1'); if (loop) p.set('loop','1'); return `https://www.youtube.com/embed/${ytMatch[1]}?${p.toString()}`; }
    const vimeoMatch = url.match(/vimeo\.com\/(\d+)/);
    if (vimeoMatch) { const p = new URLSearchParams(); if (autoplay) p.set('autoplay','1'); if (muted) p.set('muted','1'); if (loop) p.set('loop','1'); return `https://player.vimeo.com/video/${vimeoMatch[1]}?${p.toString()}`; }
...
export const VideoEmbedBlock: UserComponent<Partial<VideoEmbedBlockProps & LayoutProps>> = (props) => {
  const { url='', aspectRatio='16:9', borderRadius=12, autoplay=false, muted=true, loop=true, opacity=1 } = props;
      ...
      >
        {embedUrl ? (
          <div style={{ position:'relative', paddingBottom:aspectRatioMap[aspectRatio], height:0 }}>
            <iframe src={embedUrl} style={{ position:'absolute', top:0, left:0, width:'100%', height:'100%', border:'none', borderRadius }} allow="autoplay; fullscreen; encrypted-media" allowFullScreen />
          </div>
        ) : (
          <div style={{ paddingBottom:aspectRatioMap[aspectRatio], position:'relative' }} className="bg-muted/30 border-2 border-dashed border-muted-foreground/20">
...
VideoEmbedBlock.craft = {
  props: { url:'', aspectRatio:'16:9', borderRadius:12, autoplay:false, muted:true, loop:true, opacity:1, ...DEFAULT_LAYOUT_PROPS },
