...
export interface SocialLink { id: string; icon: string; label: string; url: string; color: string; }
    ...
    <div style={{ ...layoutStyle, cursor: 'move', pointerEvents: 'auto' }}>
      ...
      >
        <div className="flex items-center justify-center" style={{
          ...
          {links.map((link) => (
            <button key={link.id} className="flex items-center gap-2 transition-all hover:scale-110 active:scale-95"
              style={{ flexDirection: layout==='vertical' ? 'row' : 'column', gap: showLabels ? 4 : 0 }}
              onClick={(e) => e.preventDefault()}>
              <div className="flex items-center justify-center rounded-xl" style={{ width:iconSize, height:iconSize, backgroundColor:link.color+'20', border:`1px solid ${link.color}40`, fontSize:iconSize*0.5 }}>
                {link.icon}
              </div>
