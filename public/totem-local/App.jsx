    ...
    const register = async () => {
      ...
      try {
        ...
        if (data.device?.id) {
          ...
          console.log(
            `[AutoRegister] ✅ ${data.registered ? "Novo dispositivo" : "Dispositivo existente"}: ${data.device.name} (${data.device.id})`,
          );
        }
      } catch (err) {
...
const FreeCanvasRenderer = React.memo(({ canvas, activeViewId, onNavigate }) => {
  ...
  const normalElements = useMemo(
    () => sorted.filter((el) => !(el.type === "avatar" && el.props?.fixedOnScreen === true)),
    [sorted],
  );
  ...
  switch (type) {
      ...
      return (
        ...
        >
          ...
          >
            ...
            <span style={{ position: "relative", zIndex: 1 }}>{p.label || "Botão"}</span>
          </button>
        </div>
      );
    }
    ...
    case "video": {
      ...
      if (isYt && !vidSrc.includes("embed")) {
        ...
        if (videoId)
          finalSrc = `https://www.youtube.com/embed/${videoId}?autoplay=${p.autoplay ? 1 : 0}&mute=${p.muted ? 1 : 0}&loop=${p.loop ? 1 : 0}`;
      }
      if (isMp4) {
        return (
          <div
      ...
      return (
        ...
        >
          ...
          {mLabel && (
            ...
            >
              <span style={{ color: mLabelColor, fontSize: fs(mLabelSize), fontWeight: 600, letterSpacing: "0.02em" }}>
                {mLabel}
              </span>
            </div>
          )}
            ...
            {links.map((l, i) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <SocialSVGIcon
                      platform={l.platform || l.label?.toLowerCase() || ""}
                      size={iconSize * 0.55}
                      color={color}
                    />
                  </div>
      ...
      pages.forEach(function(p) {
        ...
        if (p.fieldOverrides && Object.keys(p.fieldOverrides).length > 0) {
          ...
          Object.entries(p.fieldOverrides).forEach(([k, v]) => {
            try {
              if (k.startsWith("__text_")) {
                ...
                const el = doc.body.querySelector(sel);
                if (el) {
                  if (v) el.setAttribute("data-navigate", v);
          ...
          const bodyAttrs = Array.from(doc.body.attributes)
            ...
            .join(" ");
          finalHtml =
            "<!DOCTYPE html><html><head>" + head + "</head><body " + bodyAttrs + ">" + body + "</body></html>";
        }

        // Inject active page script if _activeDataPage is set
        const activePageScript = p._activeDataPage
          ? `<script>
(function(){
      ...
      return (
        <div
          style={{
            ...
            display: "flex",
            flexDirection: "column",
            background: p.bgColor || "rgba(0,0,0,0.5)",
            borderRadius: p.borderRadius || 16,
            padding: 16,
          }}
        >
          {p.title && (
            <h2
              style={{ color: p.titleColor || "#fff", fontSize: p.titleSize || 24, fontWeight: 700, marginBottom: 12 }}
            >
              {p.title}
            </h2>
          )}
          ...
          >
            {catItems.map((item, i) => (
              ...
              >
                {item.badge && (
                  <div
                    style={{
                      ...
                      fontWeight: 700,
                      color: "#fff",
                      background: item.badgeColor || catAccent,
                    }}
                  >
                    {item.badge}
                  </div>
                ...
                {item.image ? (
                  <div style={{ aspectRatio: p.imageAspect || "4/3", width: "100%", overflow: "hidden" }}>
                    <img
                      src={item.image}
                      alt={item.name}
                      style={{ width: "100%", height: "100%", objectFit: "cover" }}
                ...
                <div style={{ padding: 12, display: "flex", flexDirection: "column", flex: 1 }}>
                  <h3 style={{ color: "#fff", fontSize: p.nameSize || 14, fontWeight: 600, lineHeight: 1.3 }}>
                    {item.name}
                  </h3>
                  {item.description && (
                    <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, marginTop: 4 }}>{item.description}</p>
                  )}
                  {p.showCategory !== false && item.category && (
                    <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 10, marginTop: 4 }}>{item.category}</span>
                  )}
                  <div style={{ flex: 1 }} />
                  {p.showPrice !== false && item.price && (
                    <p style={{ color: catPriceColor, fontSize: p.priceSize || 16, fontWeight: 700, marginTop: 8 }}>
                      {item.price}
                    </p>
                  )}
                </div>
          ...
          {catItems.length === 0 && (
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 12, textAlign: "center", padding: 32 }}>
              Nenhum produto
            </p>
          )}
        </div>
        ...
        >
          ...
          <div
            style={{
              ...
              width: 120,
              height: 120,
              background: `radial-gradient(circle, ${pixAccent}20 0%, transparent 70%)`,
              borderRadius: "50%",
              pointerEvents: "none",
            }}
          />
          <div
            style={{
              ...
              gap: 8,
              padding: "6px 16px",
              background: `${pixAccent}18`,
              borderRadius: 999,
              border: `1px solid ${pixAccent}33`,
            }}
          >
          ...
          <div
            style={{
              ...
              borderRadius: 16,
              padding: 3,
              background: `conic-gradient(from 0deg, ${pixAccent}, ${pixAccent}44, ${pixAccent})`,
            }}
          >
            <div
              style={{
          ...
          <span style={{ color: `${p.textColor || "#fff"}88`, fontSize: 13, textAlign: "center", fontWeight: 500 }}>
            {pixRecipient}
          </span>
          {p.showAmount !== false && (
            <div
              style={{
                padding: "8px 24px",
                background: `linear-gradient(135deg, ${pixAccent}22, ${pixAccent}11)`,
                borderRadius: 12,
                border: `1px solid ${pixAccent}33`,
              }}
            >
    ...
    case "bigcta": {
      ...
      return (
        ...
        >
          ...
          >
            {p.label || "Toque para começar"}
          </span>
          {p.sublabel && (
            <span
              style={{
                color: `${ctaText}77`,
                fontSize: p.sublabelSize || 13,
                textAlign: "center",
                position: "relative",
                zIndex: 1,
              }}
        ...
        {filtered.map((store, sIdx) => (
          ...
          >
            ...
            {expandedId === store.id && (
              ...
              >
                {store.description && (
                  <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, marginTop: 10, lineHeight: 1.6 }}>
                    {store.description}
                  </div>
                )}
                <div style={{ display: "flex", flexDirection: "column", gap: 5, marginTop: 10 }}>
                  {store.hours && (
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,0.6)" }}>🕐 {store.hours}</span>
                  )}
                  {store.phone && (
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,0.6)" }}>📞 {store.phone}</span>
                  )}
                  {store.floor && (
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,0.6)" }}>📍 {store.floor}</span>
                  )}
                  {store.category && (
                    <span style={{ fontSize: 11, color: accentColor, fontWeight: 600 }}>🏷️ {store.category}</span>
                  )}
...
function StoreDirectory({ props }) {
  return <PremiumStoreDirectory props={props} />;
}
...
function ChatElement({ props: p, deviceId }) {
  ...
  return (
    ...
    >
      ...
      >
        <div
          style={{ width: 8, height: 8, borderRadius: 999, background: "#4ade80", animation: "pulse 2s infinite" }}
        />
    ...
    >
      <div
        style={{
          ...
          width: "60%",
          height: "40%",
          background: `radial-gradient(ellipse, ${c}15 0%, transparent 70%)`,
          pointerEvents: "none",
        }}
      />
      <p
        style={{
          ...
          textShadow: `0 0 40px ${c}25, 0 2px 4px rgba(0,0,0,0.3)`,
          position: "relative",
        }}
      >
        {prefix}
...
function LiveForm({ props: p }) {
    ...
    return (
      ...
      >
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ color: p.titleColor || "#fff", fontSize: 18, fontWeight: 600, marginTop: 12 }}>
            {p.successMessage || "Enviado com sucesso!"}
          </p>
          <button
            ...
            style={{
              ...
              fontSize: 13,
              fontWeight: 600,
              background: `${submitBg}20`,
              color: submitBg,
              border: `1px solid ${submitBg}33`,
              cursor: "pointer",
            }}
          >
            Enviar outro
  ...
  const fieldStyle = {
    background: `linear-gradient(135deg, ${fieldBg}, rgba(255,255,255,0.04))`,
    color: fieldText,
    border: "1px solid rgba(255,255,255,0.08)",
    width: "100%",
    height: 44,
  ...
  return (
    ...
    >
      <h2 style={{ color: p.titleColor || "#fff", fontSize: p.titleSize || 22, fontWeight: 700, marginBottom: 20 }}>
        {title}
      </h2>
            ...
            <div key={field.id} style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              {field.type === "textarea" && (
                <textarea
                  ...
                  rows={3}
                  data-var={varName}
                  style={{ ...fieldStyle, height: "auto", padding: "12px 16px", resize: "none" }}
                />
              )}
              {field.type === "select" && (
                <select data-var={varName} style={fieldStyle}>
              ...
              {field.type === "checkbox" && (
                <label style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 0", cursor: "pointer" }}>
                  <input type="checkbox" data-var={varName} style={{ width: 20, height: 20, accentColor: submitBg }} />
                  <span style={{ color: fieldText, fontSize: 14 }}>{field.placeholder || field.label}</span>
                </label>
              )}
            </div>
          );
      ...
      <button
        ...
        style={{
          ...
          fontSize: 16,
          marginTop: 20,
          background: `linear-gradient(135deg, ${submitBg}, ${submitBg}dd)`,
          color: submitText,
    ...
    >
      ...
      <div
        style={{
          ...
          borderRadius: (p.borderRadius || 20) - 8,
          pointerEvents: "none",
        }}
      />
      <span
...
function LiveNumpad({ props: p }) {
  ...
  return (
    ...
    >
      <span
        style={{ color: `${textColor}99`, fontSize: 14, fontWeight: 600, textAlign: "center", letterSpacing: "0.02em" }}
      >
        {p.label || "Digite seu CPF"}
      </span>
      <div
        style={{
          padding: "14px 16px",
      ...
      >
        {display || p.placeholder || "000.000.000-00"}
      </div>
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
          ...
          <div
            style={{
              ...
              inset: 0,
              background:
                "linear-gradient(to top, rgba(0,0,0,0.95) 0%, rgba(0,0,0,0.4) 40%, rgba(0,0,0,0.2) 60%, rgba(0,0,0,0.6) 100%)",
              pointerEvents: "none",
            }}
          />
        </div>
        ...
        {filteredPosts.map((post) => {
          ...
          return (
            ...
            >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px" }}>
                  <div
                    style={{
                      ...
                      overflow: "hidden",
                      flexShrink: 0,
                      background: `linear-gradient(135deg, ${accentColor}, #ec4899, #8b5cf6)`,
                      padding: 2,
                    }}
                  >
                    <div
              ...
              {showLikes && (
                <p style={{ padding: "0 12px", color: textColor, fontSize: 11, fontWeight: 600 }}>
              ...
              <div style={{ padding: "4px 12px 12px" }}>
                ...
                {post.address && (
                  <p style={{ color: textColor, opacity: 0.5, fontSize: 10, marginTop: 4 }}>📍 {post.address}</p>
                )}
              </div>
            </div>
          );
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "0 16px 16px", marginTop: -8 }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <h3 style={{ color: textColor, fontSize: 14, fontWeight: 700, lineHeight: 1.2 }}>
                {post.title || post.author || "Loja"}
              </h3>
              {post.author && post.title && (
                <p style={{ color: textColor, opacity: 0.6, fontSize: 11, marginTop: 2 }}>{post.author}</p>
              )}
              {stars > 0 && (
                <div style={{ display: "flex", alignItems: "center", gap: 2, marginTop: 4 }}>
                  {Array.from({ length: 5 }).map((_, i) => (
                    <span key={i} style={{ color: i < stars ? "#facc15" : "rgba(255,255,255,0.2)", fontSize: 12 }}>
                      {i < stars ? "★" : "☆"}
                    </span>
                  ))}
          ...
          {(post.detailDescription || post.description) && (
            <p style={{ color: textColor, opacity: 0.8, fontSize: 11, lineHeight: 1.5, marginTop: 12 }}>
              {post.detailDescription || post.description}
            </p>
          )}
...
function UpdateToast({ visible }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "#f1f5f9", lineHeight: 1.2 }}>Configuração atualizada</div>
        <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>Interface sincronizada</div>
        <div
          style={{
            height: 2,
            borderRadius: 999,
